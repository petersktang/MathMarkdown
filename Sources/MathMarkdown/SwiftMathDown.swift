//
//  SwiftMathDownosaur.swift
//  
//
//  Created by Peter Tang on 13/9/2023.
//
...
public struct SwiftMathDown: MarkupVisitor {
    public typealias Result = NSAttributedString

    private let baseFontSize: CGFloat
    private let imageLoader: ((Markup) -> ImageResult)?

    public init(baseFontSize: CGFloat = 15.0, imageLoader: ((Markup) -> ImageResult)? = nil) {
    ...
    mutating public func visitInlineCode(_ inlineCode: InlineCode) -> NSAttributedString {
        ...
        guard let _ = UUID(uuidString: inlineCode.code), let iLoader = imageLoader?(inlineCode) else {
            return defaultInlineCode(inlineCode.code)
        }
        let (error, image, inlineCodeOriginal) = (iLoader.error, iLoader.image, iLoader.source)
        if let errorDescription = error?.localizedDescription {
            return packError(defaultInlineCode(inlineCodeOriginal), errorDescription: errorDescription)
        } else {
            return packImage(image: image)
        }
    }
    private func packError(_ defaultString: NSAttributedString, errorDescription: String, finalTouch: ((NSMutableAttributedString) -> (Void))? = nil) -> NSAttributedString {
        let resultString = NSMutableAttributedString()
        let errorString = NSAttributedString(string: " ("+errorDescription+") ", attributes: [.foregroundColor: SwiftMathColor.systemRed])
        resultString.append(defaultString)
        resultString.append(errorString)
        finalTouch?(resultString)
        return resultString
    }
    private func packImage(image: SwiftMathImage?, alignment: NSTextAlignment? = nil, finalTouch: ((NSMutableAttributedString) -> (Void))? = nil) -> NSAttributedString {
        guard let image = image else { return .init() }
        // create our NSTextAttachment
        let imageAttachment = NSTextAttachment()
        imageAttachment.bounds.size = image.size
        #if os(macOS)
        // Method1: This does not work
        // imit change to PDF rendering at the moment until we can check image on a NSTextView
        // see change in NSAttributedString.PDF)
        imageAttachment.attachmentCell = NSTextAttachmentCell(imageCell: image)
        // Method2: This does not work
        // imageAttachment.attachmentCell = NSTextAttachmentCell(imageCell: image
        //     .flipped(flipHorizontally: false, flipVertically: true)
        // )
        // Method3: This works well
        // imageAttachment.attachmentCell = NSTextAttachmentCell(imageCell: NSImage(size: image.size.integral, flipped: true) { rect in
        //     image.draw(in: rect, from: .zero, operation: .sourceOver, fraction: 1.0)
        //     return true
        // })
        // Method4: This also works well
        // imageAttachment.image = NSImage(size: image.size.integral, flipped: true) { rect in
        //     image.draw(in: rect, from: .zero, operation: .sourceOver, fraction: 1.0)
        //     return true
        // }
        // Method5: Test for Cell
        // imageAttachment.attachmentCell = NSTextAttachmentCell(imageCell: image)
        #elseif os(iOS)
        imageAttachment.image = image
        #endif
        // imageAttachment.bounds = CGRect(), add in wider bounds to hold image?
    ...
    public func visitCodeBlock(_ codeBlock: CodeBlock) -> NSAttributedString {
        ...
        guard codeBlock.language == "math", let iLoader = imageLoader?(codeBlock) else { return defaultCodeBlock(codeBlock.code) }
        let (error, image, codeBlockOriginal) = (iLoader.error, iLoader.image, iLoader.source)
        if let errorDescription = error?.localizedDescription {
            let baseFontSize = baseFontSize
            return packError(defaultCodeBlock(codeBlockOriginal), errorDescription: " "+errorDescription+"\n") { result in
                if codeBlock.hasSuccessor {
            ...
            let baseFontSize = baseFontSize
            return packImage(image: image, alignment: .center) { result in
                if codeBlock.hasSuccessor {
                    result.append(codeBlock.isContainedInList ? .singleNewline(withFontSize: baseFontSize) : .doubleNewline(withFontSize: baseFontSize))
                }
            }
    ...
    public func visitImage(_ image: Image) -> NSAttributedString {
        ...
        guard let iLoader = imageLoader?(image) else { return defaultImage(image) }
        let (iLoader_error, iLoader_image) = (iLoader.error, iLoader.image)
        if let errorDescription = iLoader_error?.localizedDescription {
            return packError(defaultImage(image), errorDescription: " "+errorDescription+"\n")
        } else if let i_image = iLoader_image {
            return packImage(image: i_image, alignment: .left)
        } else {
